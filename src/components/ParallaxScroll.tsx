    ...
    <div className="relative h-screen w-screen overflow-hidden font-mono text-white uppercase">
      {images.map((src, i) => (
        <div
          ...
          className="parallax-slide absolute top-0 left-0 h-screen w-screen bg-cover bg-center transition-transform duration-[1200ms] will-change-transform"
          style={{ backgroundImage: `url(${src})`, zIndex: totalSlides - i }}
        >
          <div className="flex h-screen items-center justify-center text-center backdrop-brightness-50">
            {renderSlide(i)}
          </div>
        </div>
      ))}
